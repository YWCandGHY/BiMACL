...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, clip):
        ...
        if random.random() < 0.5:
            # 判断处理的图片是PIL格式，还是numpy格式，[0]表示取batch中的第一个图片
...
class RandomResize(object):
    ...
    def __call__(self, clip):
        # random.uniform(x, y) 方法将随机生成一个实数，它在 [x,y] 范围内：即[3/4=0.75，4/3=1.33]之间
        scaling_factor = random.uniform(self.ratio[0], self.ratio[1])
        # 同样区分np和PIL两种形式，np模式图片的格式为(h,w,c)，而PIL为(w,h),一定要注意！！！
        if isinstance(clip[0], np.ndarray):
...
class Resize(object):
    ...
    def __call__(self, clip):
        # size (tuple): (wide, height)， 只有一个值的话就是长宽相同
        resized = F.resize_clip(
...
class RandomCrop(object):
    ...
    def __call__(self, clip):
        ...
        # 几个参数有：clip也即原始图片，y1，x1也即剪裁点的左上角坐标，h，w也即剪裁图片的长宽大小
