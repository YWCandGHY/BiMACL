...
class ColorJitter(object):
    ...
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0, grayscale=0, consistent=False, shuffle=True,
                 gray_first=True, is_split=False):
...
class KineticsResizedCrop(object):
    ...
    def _get_random_crop(self, clip):
        ...
        if clip_height < clip_width:
            ...
            new_clip_width = int(clip_width / clip_height * new_clip_height)
            new_clip = torch.nn.functional.interpolate(
                clip, size=(new_clip_height, new_clip_width), mode="bilinear", align_corners=True)
        else:
...
class KineticsResizedCropFewshot(object):
    ...
    def _get_controlled_crop(self, clip):
        ...
        if len(self.short_side_range) > 1:
            ...
            new_clip = torch.nn.functional.interpolate(
                clip, size=(self.short_side_range[0], self.short_side_range[1]), mode="bilinear", align_corners=True)
        else:
            new_clip_width = self.short_side_range[0]
            new_clip_height = self.short_side_range[0]
            new_clip = torch.nn.functional.interpolate(
                clip, size=(self.short_side_range[0], self.short_side_range[0]), mode="bilinear", align_corners=True)
        x_max = int(new_clip_width - self.crop_size)
        y_max = int(new_clip_height - self.crop_size)
        if self.num_spatial_crops == 1:
            x = x_max // 2
...
#                     ),
#                 ]
# transform = Compose(std_transform_list)
# transform_query = Compose(std_transform_list_query)